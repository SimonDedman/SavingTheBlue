...
ddet <- mydets %>%
  ...
  dplyr::filter( # locations that were user specified within the Wildlife Data Portal
    !Type %in% c("User")
  ) %>%
...
ddet %<>%
  ...
  dplyr::full_join( # add tagging info
    tags
  ) %>%
...
ddet$tdiff.days <- unlist(tapply(ddet$date,
  INDEX = ddet$id,
...
ddet_clean <- ddet %>%
  dplyr::group_by(id) %>%
  dplyr::mutate(id = paste0(id, ".", 1 + cumsum(tdiff.days >= 10)))
